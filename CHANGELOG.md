...
- Country aggregation no longer drops every responder country from RDP
  brute-force events, every originator country from external DDoS events, or
  all but one responder country from unusual destination-pattern events. Each
  event is now counted once per distinct country it carries, so aggregation
  agrees with country filtering.
