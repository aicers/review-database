...
//!
//! Event `*FieldsStored` `time` fields persist [`jiff::Timestamp`] values with
//! the [`timestamp::ts_nanoseconds`](super::timestamp::ts_nanoseconds) adapter,
//! which writes a signed 64-bit Unix-epoch nanosecond count. With
//! `bincode::serialize` (fixint encoding) each such field occupies eight
...
//! Each timestamp-bearing threat kind also has a checked-in raw binary
//! fixture that the current stored struct must decode, and whose bytes the
//! production `convert_for_storage` path must reproduce exactly.
//!
...
fn network_threat_fields(time: Timestamp) -> NetworkThreatFields {
    NetworkThreatFields {
        time,
        sensor: "sensor".to_string(),
        orig_addr: "127.0.0.1".parse().expect("valid ip"),
...
#[test]
fn extra_threat_stored_fixture_decodes_to_current_jiff_type() {
    const FIXTURE: &[u8] =
...
#[test]
fn http_threat_stored_fixture_decodes_to_current_jiff_type() {
    const FIXTURE: &[u8] =
        include_bytes!("../../tests/fixtures/event_http_threat_pre_1970_stored.bin");
    let expected = pre_1970_timestamp();
    let expected_nanos = timestamp::to_i64_nanos(expected).expect("in range");

    assert_eq!(FIXTURE.len(), 277);
    assert_eight_byte_i64_contract(&FIXTURE[..8], expected_nanos);
...
#[test]
fn http_threat_production_bytes_match_stored_fixture() {
    const FIXTURE: &[u8] =
        include_bytes!("../../tests/fixtures/event_http_threat_pre_1970_stored.bin");
    let time = pre_1970_timestamp();
    let producer_bytes =
...
#[test]
fn network_threat_stored_fixture_decodes_to_current_jiff_type() {
    const FIXTURE: &[u8] =
        include_bytes!("../../tests/fixtures/event_network_threat_pre_1970_stored.bin");
    let expected = pre_1970_timestamp();
    let expected_nanos = timestamp::to_i64_nanos(expected).expect("in range");
...
#[test]
fn network_threat_production_bytes_match_stored_fixture() {
    const FIXTURE: &[u8] =
        include_bytes!("../../tests/fixtures/event_network_threat_pre_1970_stored.bin");
    let time = pre_1970_timestamp();
    let producer_bytes =
...
#[test]
fn windows_threat_stored_fixture_decodes_to_current_jiff_type() {
    const FIXTURE: &[u8] =
        include_bytes!("../../tests/fixtures/event_windows_threat_pre_1970_stored.bin");
    let expected = pre_1970_timestamp();
    let expected_nanos = timestamp::to_i64_nanos(expected).expect("in range");

    assert_eq!(FIXTURE.len(), 173);
    assert_eight_byte_i64_contract(&FIXTURE[..8], expected_nanos);
...
#[test]
fn windows_threat_production_bytes_match_stored_fixture() {
    const FIXTURE: &[u8] =
        include_bytes!("../../tests/fixtures/event_windows_threat_pre_1970_stored.bin");
    let time = pre_1970_timestamp();
    let producer_bytes =
...
#[test]
fn extra_threat_production_bytes_match_stored_fixture() {
    const FIXTURE: &[u8] =
        include_bytes!("../../tests/fixtures/event_extra_threat_pre_1970_stored.bin");
    let time = pre_1970_timestamp();
    let producer_bytes =
...
#[test]
#[ignore = "rewrites the checked-in binary fixture; run only when intentionally replacing this baseline fixture"]
fn regenerate_http_threat_pre_1970_stored_fixture() {
    let time = pre_1970_timestamp();
    let producer_bytes =
        bincode::serialize(&http_threat_fields(time)).expect("serializable producer fields");
    let bytes = convert_for_storage(EventKind::HttpThreat, &producer_bytes, None)
        .expect("convertible to stored fields");
    let fixture_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/event_http_threat_pre_1970_stored.bin");
...
#[test]
#[ignore = "rewrites the checked-in binary fixture; run only when intentionally replacing this baseline fixture"]
fn regenerate_network_threat_pre_1970_stored_fixture() {
    let time = pre_1970_timestamp();
    let producer_bytes =
        bincode::serialize(&network_threat_fields(time)).expect("serializable producer fields");
    let bytes = convert_for_storage(EventKind::NetworkThreat, &producer_bytes, None)
        .expect("convertible to stored fields");
    let fixture_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/event_network_threat_pre_1970_stored.bin");
...
#[test]
#[ignore = "rewrites the checked-in binary fixture; run only when intentionally replacing this baseline fixture"]
fn regenerate_windows_threat_pre_1970_stored_fixture() {
    let time = pre_1970_timestamp();
    let producer_bytes =
        bincode::serialize(&windows_threat_fields(time)).expect("serializable producer fields");
    let bytes = convert_for_storage(EventKind::WindowsThreat, &producer_bytes, None)
        .expect("convertible to stored fields");
    let fixture_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/event_windows_threat_pre_1970_stored.bin");
