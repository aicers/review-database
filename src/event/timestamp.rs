...
mod tests {
    ...
    #[derive(serde::Serialize, serde::Deserialize, PartialEq, Eq, Debug)]
    struct ChronoTsField {
        #[serde(with = "chrono::serde::ts_nanoseconds")]
        time: DateTime<Utc>,
    }

    #[test]
    fn serde_adapter_matches_chrono_ts_nanoseconds_bincode_bytes() {
        let epoch = 1_592_224_245_000_000_000;
        let cases = [
            i64::MIN,
            i64::MIN + 1,
            -1_592_224_245_987_654_321,
            -1,
            0,
            1,
            999_999_999,
        ...
        for nanos in cases {
            let chrono_time = DateTime::<Utc>::from_timestamp_nanos(nanos);
            let jiff_time = from_i64_nanos(nanos).expect("i64 nanos fit Jiff timestamp range");

            let chrono_bytes = bincode::serialize(&ChronoTsField { time: chrono_time })
                .expect("chrono timestamp should serialize as i64 nanoseconds");
            let jiff_bytes = bincode::serialize(&TsField { time: jiff_time })
                .expect("Jiff timestamp should serialize as i64 nanoseconds");

            assert_eq!(chrono_bytes, jiff_bytes);
            assert_eq!(
                chrono_bytes,
                bincode::serialize(&nanos).expect("i64 nanoseconds should serialize")
            );

            let decoded_jiff: TsField =
                bincode::deserialize(&chrono_bytes).expect("chrono bytes should decode as Jiff");
            assert_eq!(decoded_jiff.time, jiff_time);

            let decoded_chrono: ChronoTsField =
                bincode::deserialize(&jiff_bytes).expect("Jiff bytes should decode as chrono");
            assert_eq!(decoded_chrono.time, chrono_time);
        }
    }
