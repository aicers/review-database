...
/// ```
const COMPATIBLE_VERSION_REQ: &str = ">=0.43.0-alpha.4,<0.43.0-alpha.5";
...
pub fn migrate_data_dir<P: AsRef<Path>>(data_dir: P, backup_dir: P) -> Result<()> {
    ...
    let migration: Vec<Migration> = vec![(
        VersionReq::parse(">=0.42.0,<0.43.0-alpha.4")?,
        Version::parse("0.43.0-alpha.4")?,
...
fn migrate_0_42_to_0_43(data_dir: &Path) -> Result<()> {
    ...
    // Step 4: Migrate network tags to customer-scoped format
