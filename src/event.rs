...
pub(crate) use self::{
    ...
    log::ExtraThreatFieldsStoredV0_46,
    ...
    sysmon::WindowsThreatFieldsStoredV0_46,
...
use super::{
    ...
    migration::migration_structures::{
        V0_46_COUNTRY_CODE_LOOKUP_FAILED, V0_46_COUNTRY_CODE_UNRESOLVED,
    },
...
impl Event {
    ...
    pub fn count_category(
        ...
        let (matched, _triage) = self.matches(filter)?;
        if !matched {
            return Ok(());
        }

        let (_kind, category) = self.kind_and_category();
...
/// attribution.
///
/// `countries` compares stored two-byte codes directly. This includes the
/// [`crate::COUNTRY_CODE_UNKNOWN`] and [`crate::COUNTRY_CODE_UNRESOLVED`]
/// placeholders, which callers may filter explicitly.
///
/// # Limitations
///
...
/// conversion so the stored value is serialized only once; lookup failures use
/// [`crate::COUNTRY_CODE_UNKNOWN`]. Without one, every endpoint uses
/// [`crate::COUNTRY_CODE_UNRESOLVED`]. Country-code vectors retain the length
/// and ordering of their corresponding address vectors.
...
/// Swaps the two country-code placeholders stored by database format 0.46.
///
/// Each code is changed according to its original bytes, so scalar and vector
/// fields are transformed in one deserialize/serialize operation without
/// changing vector length or order. Real country codes are left untouched.
pub(crate) fn swap_stored_country_code_placeholders(
    kind: EventKind,
    bytes: &[u8],
) -> Result<Vec<u8>> {
    fn swap(code: &mut [u8; 2]) {
        *code = match *code {
            V0_46_COUNTRY_CODE_UNRESOLVED => crate::util::COUNTRY_CODE_UNRESOLVED,
            V0_46_COUNTRY_CODE_LOOKUP_FAILED => crate::util::COUNTRY_CODE_UNKNOWN,
            country => country,
        };
    }

    fn reserialize<T, F>(bytes: &[u8], update: F) -> Result<Vec<u8>>
    where
        T: for<'de> Deserialize<'de> + Serialize,
        F: FnOnce(&mut T),
    {
        let mut fields: T = bincode::deserialize(bytes)
            .context("failed to deserialize stored event fields for placeholder migration")?;
        update(&mut fields);
        bincode::serialize(&fields)
            .context("failed to serialize stored event fields after placeholder migration")
    }

    macro_rules! pair {
        ($ty:ty) => {
            reserialize::<$ty, _>(bytes, |fields| {
                swap(&mut fields.orig_country_code);
                swap(&mut fields.resp_country_code);
            })
        };
    }
    macro_rules! resp_vec {
        ($ty:ty) => {
            reserialize::<$ty, _>(bytes, |fields| {
                swap(&mut fields.orig_country_code);
                fields.resp_country_codes.iter_mut().for_each(swap);
            })
        };
    }
    ...
    match kind {
        ...
        EventKind::ExternalDdos => reserialize::<ExternalDdosFieldsStored, _>(bytes, |fields| {
            fields.orig_country_codes.iter_mut().for_each(swap);
            swap(&mut fields.resp_country_code);
        }),
...
mod tests {
    ...
    impl crate::geo::CountryLookup for FakeCountryLookup {
        fn lookup_country_code(&self, addr: IpAddr) -> [u8; 2] {
            if self.failures.contains(&addr) {
                return crate::util::COUNTRY_CODE_UNKNOWN;
            }
            self.codes
                .get(&addr)
                .copied()
    ...
    fn event_db_put() {
        ...
        assert_eq!(
            stored.orig_country_code,
            crate::util::COUNTRY_CODE_UNRESOLVED
        );
        assert_eq!(
            stored.resp_country_code,
            crate::util::COUNTRY_CODE_UNRESOLVED
        );
    ...
    #[test]
    fn event_db_put_uses_unknown_code_after_attempted_lookup_failure() {
        let orig_addr = IpAddr::V4(Ipv4Addr::LOCALHOST);
        let lookup = FakeCountryLookup {
            codes: HashMap::new(),
        ...
        let (_, bytes) = db.raw_iter().next().unwrap().unwrap();
        let stored: super::DnsEventFieldsStored = bincode::deserialize(&bytes).unwrap();
        assert_eq!(stored.orig_country_code, crate::util::COUNTRY_CODE_UNKNOWN);
        assert_eq!(stored.resp_country_code, crate::util::COUNTRY_CODE_UNKNOWN);
    }
    ...
    #[test]
    fn placeholder_swap_updates_scalar_codes_from_original_bytes() {
        let msg = example_message(
            EventKind::DnsCovertChannel,
            EventCategory::CommandAndControl,
        );
        let current =
            super::convert_for_storage(EventKind::DnsCovertChannel, &msg.fields, None).unwrap();
        let mut historical: super::DnsEventFieldsStored = bincode::deserialize(&current).unwrap();
        historical.orig_country_code =
            crate::migration::migration_structures::V0_46_COUNTRY_CODE_UNRESOLVED;
        historical.resp_country_code =
            crate::migration::migration_structures::V0_46_COUNTRY_CODE_LOOKUP_FAILED;

        let swapped = super::swap_stored_country_code_placeholders(
            EventKind::DnsCovertChannel,
            &bincode::serialize(&historical).unwrap(),
        )
        .unwrap();
        let stored: super::DnsEventFieldsStored = bincode::deserialize(&swapped).unwrap();
        assert_eq!(stored.orig_country_code, crate::COUNTRY_CODE_UNRESOLVED);
        assert_eq!(stored.resp_country_code, crate::COUNTRY_CODE_UNKNOWN);
    }

    #[test]
    fn placeholder_swap_preserves_vector_length_order_and_real_codes() {
        let fields = MultiHostPortScanFields {
            sensor: "collector1".to_string(),
            orig_addr: IpAddr::V4(Ipv4Addr::LOCALHOST),
            resp_port: 443,
        ...
        let current = super::convert_for_storage(
            EventKind::MultiHostPortScan,
            &bincode::serialize(&fields).unwrap(),
            None,
        )
        .unwrap();
        let mut historical: super::MultiHostPortScanFieldsStored =
            bincode::deserialize(&current).unwrap();
        historical.orig_country_code =
            crate::migration::migration_structures::V0_46_COUNTRY_CODE_LOOKUP_FAILED;
        historical.resp_country_codes = vec![
            crate::migration::migration_structures::V0_46_COUNTRY_CODE_UNRESOLVED,
            *b"US",
            crate::migration::migration_structures::V0_46_COUNTRY_CODE_LOOKUP_FAILED,
        ];
        ...
        .unwrap();
        let stored: super::MultiHostPortScanFieldsStored = bincode::deserialize(&swapped).unwrap();
        assert_eq!(stored.orig_country_code, crate::COUNTRY_CODE_UNKNOWN);
        assert_eq!(
            stored.resp_country_codes,
            vec![
    ...
    #[test]
    fn placeholder_swap_recognizes_every_v0_46_event_kind() {
        for (kind, historical) in super::stored_event_samples_v0_46() {
            let current = super::swap_stored_country_code_placeholders(kind, &historical)
                .unwrap_or_else(|error| panic!("failed to swap {kind:?}: {error:#}"));

            if !matches!(kind, EventKind::ExtraThreat | EventKind::WindowsThreat) {
                assert_ne!(current, historical, "{kind:?} must contain a placeholder");
            }
        }
    }
    ...
    fn syslog_for_portscan() {
        ...
        assert_eq!(
            &port_scan,
            r#"time="1970-01-01T00:01:01+00:00" event_kind="PortScan" category="Reconnaissance" orig_addr="127.0.0.1" orig_country_code="XX" resp_addr="127.0.0.2" resp_country_code="XX" resp_ports="80,443,8000,8080,8888,8443,9000,9001,9002" first_event_start_time="1970-01-01T00:01:01+00:00" last_event_start_time="1970-01-01T00:01:02+00:00" proto="6" triage_scores="""#
        );
    }
    ...
    fn syslog_for_externalddos() {
        ...
        assert_eq!(
            &external_ddos,
            r#"time="1970-01-01T01:01:01+00:00" event_kind="ExternalDdos" category="Impact" orig_addrs="127.0.0.2,127.0.0.3" orig_country_codes="XX,XX" resp_addr="127.0.0.1" resp_country_code="XX" proto="6" first_event_start_time="1970-01-01T00:01:01+00:00" last_event_start_time="1970-01-01T00:01:02+00:00" triage_scores="""#
        );
    }
    ...
    fn syslog_for_ftpbruteforce() {
        ...
        assert_eq!(
            &ftp_brute_force,
            r#"time="1970-01-01T00:01:01+00:00" event_kind="FtpBruteForce" category="CredentialAccess" orig_addr="127.0.0.1" orig_country_code="XX" resp_addr="127.0.0.2" resp_port="21" resp_country_code="XX" proto="6" user_list="user1,user_2" first_event_start_time="1970-01-01T00:01:01+00:00" last_event_start_time="1970-01-01T00:01:02+00:00" is_internal="true" triage_scores="""#
        );
    }
    ...
    fn syslog_for_repeatedhttpsessions() {
        ...
        assert_eq!(
            ...
            r#"time="1970-01-01T01:01:01+00:00" event_kind="RepeatedHttpSessions" category="Exfiltration" sensor="collector1" orig_addr="127.0.0.1" orig_port="10000" orig_country_code="XX" resp_addr="127.0.0.2" resp_port="443" resp_country_code="XX" proto="6" first_event_start_time="1970-01-01T01:01:01+00:00" last_event_start_time="1970-01-01T01:01:01+00:00" triage_scores="""#
        );
    }
    ...
    fn syslog_for_rdpbruteforce() {
        ...
        assert_eq!(
            &rdp_brute_force,
            r#"time="1970-01-01T00:01:01+00:00" event_kind="RdpBruteForce" category="Discovery" orig_addr="127.0.0.1" orig_country_code="XX" resp_addrs="127.0.0.2,127.0.0.3" resp_country_codes="XX,XX" first_event_start_time="1970-01-01T00:01:01+00:00" last_event_start_time="1970-01-01T00:10:02+00:00" proto="6" triage_scores="""#
        );
    }
